...
public class ClassMap {
   private static final ClassMap.CacheMiss CACHE_MISS = new ClassMap.CacheMiss();
   ...
   private static Method[] getAccessibleMethods(Class clazz) {
      ...
      if (Modifier.isPublic(clazz.getModifiers())) {
         ...
         int upcastCount;
         for(upcastCount = methods.length; upcastCount-- > 0; methodInfos[upcastCount] = new ClassMap.MethodInfo(methods[upcastCount])) {
         }
         ...
         for(int i = 0; i < methodInfos.length; ++i) {
            ClassMap.MethodInfo methodInfo = methodInfos[i];
