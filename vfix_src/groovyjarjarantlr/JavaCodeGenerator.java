...
public class JavaCodeGenerator extends CodeGenerator {
   ...
   public void gen(LexerGrammar var1) throws IOException {
      ...
      try {
         ...
         if (this.grammar.superClass != null) {
            ...
            var3 = "groovyjarjarantlr." + this.grammar.getSuperClass();
         }
   ...
   public void gen(OneOrMoreBlock var1) {
      ...
      try {
         ...
         if (var7) {
            ...
            this.println("if ( " + var3 + ">=1 && " + var9 + ") break " + var4 + ";", -888);
         }

         JavaBlockFinishingInfo var13 = this.genCommonBlock(var1, false);
         this.genBlockFinish(var13, "if ( " + var3 + ">=1 ) { break " + var4 + "; } else {" + this.throwNoViable + "}", var1.getLine());
         this.println(var3 + "++;");
         --this.tabs;
   ...
   public JavaBlockFinishingInfo genCommonBlock(AlternativeBlock var1, boolean var2) {
      ...
      try {
         ...
         if (!var1.not || !this.analyzer.subruleCanBeInverted(var1, this.grammar instanceof LexerGrammar)) {
            ...
            int var30;
            for(var30 = 0; var30 < var1.getAlternatives().size(); ++var30) {
               Alternative var31 = var1.getAlternativeAt(var30);
               if (suitableForCaseExpression(var31)) {
                  ++var29;
               }
            }
            ...
            if (var29 >= this.makeSwitchThreshold) {
               var11 = this.lookaheadString(1);
               var5 = true;
               if (this.grammar instanceof TreeWalkerGrammar) {
                  this.println("if (_t==null) _t=ASTNULL;");
               }

               this.println("switch ( " + var11 + ") {");
            ...
            var30 = this.grammar instanceof LexerGrammar ? this.grammar.maxk : 0;

            int var34;
            for(var32 = var30; var32 >= 0; --var32) {
               if (this.DEBUG_CODE_GENERATOR) {
                  System.out.println("checking depth " + var32);
               }

               for(var34 = 0; var34 < var1.alternatives.size(); ++var34) {
                  Alternative var36 = var1.getAlternativeAt(var34);
                  if (this.DEBUG_CODE_GENERATOR) {
                     System.out.println("genAlt: " + var34);
                  }
                  ...
                  if (var5 && suitableForCaseExpression(var36)) {
                     ...
                     String var17;
                     if (this.grammar instanceof LexerGrammar) {
                        var16 = var36.lookaheadDepth;
                        if (var16 == Integer.MAX_VALUE) {
                           var16 = this.grammar.maxk;
                        }

                        while(var16 >= 1 && var36.cache[var16].containsEpsilon()) {
                           --var16;
                        }

                        if (var16 != var32) {
                           if (this.DEBUG_CODE_GENERATOR) {
                              System.out.println("ignoring alt because effectiveDepth!=altDepth;" + var16 + "!=" + var32);
                           }
                           continue;
                        }

                        var15 = this.lookaheadIsEmpty(var36, var16);
                        var17 = this.getLookaheadTestExpression(var36, var16);
                     } else {
                        var15 = this.lookaheadIsEmpty(var36, this.grammar.maxk);
                        var17 = this.getLookaheadTestExpression(var36, this.grammar.maxk);
                     }

                     var16 = this.defaultLine;

                     try {
                        this.defaultLine = var36.head.getLine();
                        if (var36.cache[1].fset.degree() > 127 && suitableForCaseExpression(var36)) {
                           if (var4 == 0) {
                              this.println("if " + var17 + " {");
                           } else {
                              this.println("else if " + var17 + " {");
                           }
                        } else if (var15 && var36.semPred == null && var36.synPred == null) {
                           if (var4 == 0) {
                              this.println("{");
                           } else {
                              this.println("else {");
                           ...
                           if (var36.semPred != null) {
                              ...
                              if ((this.grammar instanceof ParserGrammar || this.grammar instanceof LexerGrammar) && this.grammar.debuggingOutput) {
                                 var17 = "(" + var17 + "&& fireSemanticPredicateEvaluated(antlr.debug.SemanticPredicateEvent.PREDICTING," + this.addSemPred(this.charFormatter.escapeString(var19)) + "," + var19 + "))";
                              } else {
                                 var17 = "(" + var17 + "&&(" + var19 + "))";
                              }
                           }
                           ...
                           if (var4 > 0) {
                              if (var36.synPred != null) {
                                 ...
                                 ++this.tabs;
                                 this.genSynPred(var36.synPred, var17);
                                 ++var6;
                              } else {
                                 this.println("else if " + var17 + " {");
                              }
   ...
   protected String getLookaheadTestTerm(int var1, BitSet var2) {
      ...
      if (elementsAreRange(var4)) {
         ...
         int var5 = var2.degree();
         if (var5 == 0) {
            return "true";
         } else {
            int var6;
            if (var5 >= this.bitsetTestThreshold) {
               var6 = this.markBitsetForGen(var2);
               return this.getBitsetName(var6) + ".member(" + var3 + ")";
            } else {
               StringBuffer var7 = new StringBuffer();

               for(var6 = 0; var6 < var4.length; ++var6) {
                  String var8 = this.getValueString(var4[var6]);
                  if (var6 > 0) {
                     var7.append("||");
                  }
