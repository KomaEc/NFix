...
public class BlockStmtContext extends AbstractJavaParserContext<BlockStmt> {
   ...
   public List<VariableDeclarator> localVariablesExposedToChild(Node child) {
      ...
      for(int i = 0; i < ((BlockStmt)this.wrappedNode).getStatements().size(); ++i) {
         if (((Statement)((BlockStmt)this.wrappedNode).getStatements().get(i)).equals(child)) {
            position = i;
         }
      }
      ...
      if (position == -1) {
         ...
         for(int i = position - 1; i >= 0; --i) {
            variableDeclarators.addAll(this.localVariablesDeclaredIn(((BlockStmt)this.wrappedNode).getStatement(i)));
         }
