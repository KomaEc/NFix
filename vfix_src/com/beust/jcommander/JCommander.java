...
public class JCommander {
   ...
   private Map<JCommander.ProgramName, JCommander> m_commands = Maps.newLinkedHashMap();
   ...
   private JCommander.ProgramName m_programName;
   ...
   private void initializeDefaultValues() {
      if (this.m_defaultProvider != null) {
         ...
         while(i$.hasNext()) {
            Entry<JCommander.ProgramName, JCommander> entry = (Entry)i$.next();
   ...
   public void setProgramName(String name, String... aliases) {
      this.m_programName = new JCommander.ProgramName(name, Arrays.asList(aliases));
   }
   ...
   public void usage(StringBuilder out, String indent) {
      ...
      if (hasCommands) {
         ...
         while(i$.hasNext()) {
            Entry<JCommander.ProgramName, JCommander> commands = (Entry)i$.next();
   ...
   public void addCommand(String name, Object object, String... aliases) {
      ...
      JCommander.ProgramName progName = jc.m_programName;
      ...
      for(int i$ = 0; i$ < len$; ++i$) {
         ...
         if (!alias.equals(name)) {
            JCommander.ProgramName mappedName = (JCommander.ProgramName)this.aliasMap.get(alias);
   ...
   public Map<String, JCommander> getCommands() {
      ...
      while(i$.hasNext()) {
         Entry<JCommander.ProgramName, JCommander> entry = (Entry)i$.next();
         res.put(((JCommander.ProgramName)entry.getKey()).m_name, entry.getValue());
      }
   ...
   private JCommander.ProgramName findProgramName(String name) {
      return (JCommander.ProgramName)FuzzyMap.findInMap(this.aliasMap, new StringKey(name), this.m_caseSensitiveOptions, this.m_allowAbbreviatedOptions);
   }
   ...
   private static final class ProgramName implements FuzzyMap.IKey {
      ...
      private String getDisplayName() {
         ...
         if (!this.m_aliases.isEmpty()) {
            sb.append("(");
            Iterator aliasesIt = this.m_aliases.iterator();
      ...
      public int hashCode() {
         int prime = true;
         int result = 1;
         int result = 31 * result + (this.m_name == null ? 0 : this.m_name.hashCode());
         return result;
      }
