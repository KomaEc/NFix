...
public class MoreTypes {
   ...
   private static boolean isFullySpecified(Type type) {
      if (type instanceof Class) {
         return true;
      } else if (type instanceof MoreTypes.CompositeType) {
         return ((MoreTypes.CompositeType)type).isFullySpecified();
      } else {
         return type instanceof TypeVariable ? false : ((MoreTypes.CompositeType)canonicalize(type)).isFullySpecified();
      }
   }
   ...
   public static Type canonicalize(Type type) {
      if (type instanceof Class) {
         Class<?> c = (Class)type;
         return (Type)(c.isArray() ? new MoreTypes.GenericArrayTypeImpl(canonicalize(c.getComponentType())) : c);
      } else if (type instanceof MoreTypes.CompositeType) {
         return type;
      } else if (type instanceof ParameterizedType) {
         ParameterizedType p = (ParameterizedType)type;
         return new MoreTypes.ParameterizedTypeImpl(p.getOwnerType(), p.getRawType(), p.getActualTypeArguments());
      } else if (type instanceof GenericArrayType) {
         GenericArrayType g = (GenericArrayType)type;
         return new MoreTypes.GenericArrayTypeImpl(g.getGenericComponentType());
      } else if (type instanceof WildcardType) {
         WildcardType w = (WildcardType)type;
         return new MoreTypes.WildcardTypeImpl(w.getUpperBounds(), w.getLowerBounds());
      } else {
         return type;
      }
   }
   ...
   public static Class<?> getRawType(Type type) {
      if (type instanceof Class) {
         ...
         String var1 = String.valueOf(String.valueOf("Expected a Class, ParameterizedType, or GenericArrayType, but <"));
         String var2 = String.valueOf(String.valueOf(type));
         String var3 = String.valueOf(String.valueOf(type.getClass().getName()));
         throw new IllegalArgumentException((new StringBuilder(13 + var1.length() + var2.length() + var3.length())).append(var1).append(var2).append("> is of type ").append(var3).toString());
      }
   }
   ...
   public static class WildcardTypeImpl implements WildcardType, Serializable, MoreTypes.CompositeType {
      ...
      public String toString() {
         ...
         if (this.lowerBound != null) {
            var10001 = String.valueOf(MoreTypes.typeToString(this.lowerBound));
            if (var10001.length() != 0) {
               var10000 = "? super ".concat(var10001);
            } else {
               var10002 = new String;
               var10000 = var10002;
               var10002.<init>("? super ");
            ...
            var10001 = String.valueOf(MoreTypes.typeToString(this.upperBound));
            if (var10001.length() != 0) {
               var10000 = "? extends ".concat(var10001);
            } else {
               var10002 = new String;
               var10000 = var10002;
               var10002.<init>("? extends ");
            }
   ...
   public static class GenericArrayTypeImpl implements GenericArrayType, Serializable, MoreTypes.CompositeType {
   ...
   public static class ParameterizedTypeImpl implements ParameterizedType, Serializable, MoreTypes.CompositeType {
