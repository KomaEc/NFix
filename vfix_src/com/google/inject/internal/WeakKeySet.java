...
final class WeakKeySet {
   ...
   private final Cache<State, Set<WeakKeySet.KeyAndSource>> evictionCache = CacheBuilder.newBuilder().weakKeys().removalListener(new RemovalListener<State, Set<WeakKeySet.KeyAndSource>>() {
      public void onRemoval(RemovalNotification<State, Set<WeakKeySet.KeyAndSource>> notification) {
   ...
   private void cleanUpForCollectedState(Set<WeakKeySet.KeyAndSource> keysAndSources) {
      synchronized(this.lock) {
         Iterator i$ = keysAndSources.iterator();

         while(i$.hasNext()) {
            WeakKeySet.KeyAndSource keyAndSource = (WeakKeySet.KeyAndSource)i$.next();
   ...
   public void add(Key<?> key, State state, Object source) {
      ...
      if (state.parent() != State.NONE) {
         Set<WeakKeySet.KeyAndSource> keyAndSources = (Set)this.evictionCache.getIfPresent(state);
         if (keyAndSources == null) {
            this.evictionCache.put(state, keyAndSources = Sets.newHashSet());
         }

         ((Set)keyAndSources).add(new WeakKeySet.KeyAndSource(key, convertedSource));
      }
   ...
   private static final class KeyAndSource {
      ...
      public boolean equals(Object obj) {
         if (this == obj) {
            return true;
         } else if (!(obj instanceof WeakKeySet.KeyAndSource)) {
            return false;
         } else {
            WeakKeySet.KeyAndSource other = (WeakKeySet.KeyAndSource)obj;
