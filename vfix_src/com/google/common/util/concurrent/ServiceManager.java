...
public final class ServiceManager {
   ...
   public ServiceManager(Iterable<? extends Service> services) {
      ...
      while(i$.hasNext()) {
         ...
         service.addListener(new ServiceManager.ServiceListener(service, stateReference), new ServiceManager.SynchronizedExecutor());
   ...
   private static final class SynchronizedExecutor implements Executor {
      ...
      // $FF: synthetic method
      SynchronizedExecutor(Object x0) {
         this();
      }
   }
   ...
   private static final class ServiceManagerState {
      ...
      void addListener(ServiceManager.Listener listener, Executor executor) {
         ...
         try {
            if (!this.stoppedGuard.isSatisfied()) {
               this.listeners.add(new ServiceManager.ListenerExecutorPair(listener, executor));
            }
         } finally {
            this.monitor.leave();
         }
      ...
      void fireStoppedListeners() {
         ...
         while(i$.hasNext()) {
            final ServiceManager.ListenerExecutorPair pair = (ServiceManager.ListenerExecutorPair)i$.next();
