...
public class WrappingProcess {
   ...
   public void start() throws IOException {
      ...
      ProcessBuilder processBuilder = createProcessBuilder(this.processArgs.getJavaExecutable(), this.processArgs.getJvmArgs(), this.slaveClass, Arrays.asList(args), this.processArgs.getJavaAgentFinder());
   ...
   private void configureProcessBuilder(ProcessBuilder processBuilder, File workingDirectory, String initialClassPath, Map<String, String> environmentVariables) {
      processBuilder.directory(workingDirectory);
      Map<String, String> environment = processBuilder.environment();
      environment.put("CLASSPATH", initialClassPath);
      Iterator i$ = environmentVariables.entrySet().iterator();
   ...
   private static void removeClassPathProperties(List<String> cmd) {
      for(int i = cmd.size() - 1; i >= 0; --i) {
         if (((String)cmd.get(i)).startsWith("-Djava.class.path")) {
            cmd.remove(i);
         }
      }
   ...
   private static void addPITJavaAgent(JavaAgent agentJarLocator, List<String> cmd) {
      ...
      Iterator i$ = jarLocation.iterator();
   ...
   private static void addLaunchJavaAgents(List<String> cmd) {
      ...
      FunctionalList<String> agents = FCollection.filter(rt.getInputArguments(), Prelude.or(isJavaAgentParam(), isEnvironmentSetting()));
      cmd.addAll(agents);
   }
