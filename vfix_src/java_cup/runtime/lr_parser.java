...
public abstract class lr_parser {
   ...
   protected void report_expected_token_ids() {
      ...
      while(var4.hasNext()) {
         Integer expected = (Integer)var4.next();
   ...
   public String symbl_name_from_id(int id) {
      Field[] fields = this.getSymbolContainer().getFields();
      Field[] var6 = fields;
   ...
   public List<Integer> expected_token_ids() {
      ...
      for(int i = 0; i < row.length; i += 2) {
         if (row[i] != -1 && this.validate_expected_symbol(row[i])) {
   ...
   private boolean validate_expected_symbol(int id) {
      try {
         ...
         do {
            while(true) {
               int act = this.get_action(vstack.top(), id);
               if (act == 0) {
                  return false;
               }
