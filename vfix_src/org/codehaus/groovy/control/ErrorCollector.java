...
public class ErrorCollector {
   ...
   public WarningMessage getWarning(int index) {
      return index < this.getWarningCount() ? (WarningMessage)this.warnings.get(index) : null;
   }

   public Message getError(int index) {
      return index < this.getErrorCount() ? (Message)this.errors.get(index) : null;
   }
   ...
   private void write(PrintWriter writer, Janitor janitor, List messages, String txt) {
      if (messages != null && messages.size() != 0) {
         for(Iterator iterator = messages.iterator(); iterator.hasNext(); writer.println()) {
