...
public abstract class AbstractMapBag implements Bag {
   ...
   boolean containsAll(Bag other) {
      ...
      boolean contains;
      for(Iterator it = other.uniqueSet().iterator(); it.hasNext(); result = result && contains) {
         Object current = it.next();
         contains = this.getCount(current) >= other.getCount(current);
      }
   ...
   public int hashCode() {
      ...
      for(Iterator it = this.map.entrySet().iterator(); it.hasNext(); total += (element == null ? 0 : element.hashCode()) ^ count.value) {
         Entry entry = (Entry)it.next();
