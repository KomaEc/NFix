...
public class GitXpp3Reader {
   ...
   private Settings parseSettings(XmlPullParser parser, boolean strict) throws IOException, XmlPullParserException {
      ...
      while((strict ? parser.nextTag() : this.nextTag(parser)) == 2) {
         if (this.checkFieldWithDuplicate(parser, "revParseDateFormat", (String)null, parsed)) {
            settings.setRevParseDateFormat(this.getTrimmedValue(parser.nextText()));
         } else if (this.checkFieldWithDuplicate(parser, "traceGitCommand", (String)null, parsed)) {
            settings.setTraceGitCommand(this.getTrimmedValue(parser.nextText()));
         } else if (this.checkFieldWithDuplicate(parser, "commitNoVerify", (String)null, parsed)) {
   ...
   private Settings read(XmlPullParser parser, boolean strict) throws IOException, XmlPullParserException {
      for(int eventType = parser.getEventType(); eventType != 1; eventType = parser.next()) {
         if (eventType == 2) {
            if (strict && !"git-settings".equals(parser.getName())) {
               throw new XmlPullParserException("Expected root element 'git-settings' but found '" + parser.getName() + "'", parser, (Throwable)null);
            }
      ...
      throw new XmlPullParserException("Expected root element 'git-settings' but found no element at all: invalid XML document", parser, (Throwable)null);
   }
