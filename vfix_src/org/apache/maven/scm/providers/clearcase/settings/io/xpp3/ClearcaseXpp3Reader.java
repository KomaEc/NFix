...
public class ClearcaseXpp3Reader {
   ...
   private Settings parseSettings(XmlPullParser parser, boolean strict) throws IOException, XmlPullParserException {
      ...
      while((strict ? parser.nextTag() : this.nextTag(parser)) == 2) {
         if (this.checkFieldWithDuplicate(parser, "viewstore", (String)null, parsed)) {
            settings.setViewstore(this.getTrimmedValue(parser.nextText()));
         } else if (this.checkFieldWithDuplicate(parser, "useVWSParameter", (String)null, parsed)) {
            settings.setUseVWSParameter(this.getBooleanValue(this.getTrimmedValue(parser.nextText()), "useVWSParameter", parser, "true"));
         } else if (this.checkFieldWithDuplicate(parser, "clearcaseType", (String)null, parsed)) {
            settings.setClearcaseType(this.getTrimmedValue(parser.nextText()));
         } else if (this.checkFieldWithDuplicate(parser, "changelogUserFormat", (String)null, parsed)) {
   ...
   private Settings read(XmlPullParser parser, boolean strict) throws IOException, XmlPullParserException {
      for(int eventType = parser.getEventType(); eventType != 1; eventType = parser.next()) {
         if (eventType == 2) {
            if (strict && !"clearcase-settings".equals(parser.getName())) {
               throw new XmlPullParserException("Expected root element 'clearcase-settings' but found '" + parser.getName() + "'", parser, (Throwable)null);
            }
      ...
      throw new XmlPullParserException("Expected root element 'clearcase-settings' but found no element at all: invalid XML document", parser, (Throwable)null);
   }
