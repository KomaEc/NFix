...
public class StarteamXpp3Reader {
   ...
   private Settings parseSettings(XmlPullParser parser, boolean strict) throws IOException, XmlPullParserException {
      ...
      while((strict ? parser.nextTag() : this.nextTag(parser)) == 2) {
         if (this.checkFieldWithDuplicate(parser, "compressionEnable", (String)null, parsed)) {
            settings.setCompressionEnable(this.getBooleanValue(this.getTrimmedValue(parser.nextText()), "compressionEnable", parser, "false"));
         } else if (this.checkFieldWithDuplicate(parser, "eol", (String)null, parsed)) {
   ...
   private Settings read(XmlPullParser parser, boolean strict) throws IOException, XmlPullParserException {
      for(int eventType = parser.getEventType(); eventType != 1; eventType = parser.next()) {
         if (eventType == 2) {
            if (strict && !"starteam-settings".equals(parser.getName())) {
               throw new XmlPullParserException("Expected root element 'starteam-settings' but found '" + parser.getName() + "'", parser, (Throwable)null);
            }
      ...
      throw new XmlPullParserException("Expected root element 'starteam-settings' but found no element at all: invalid XML document", parser, (Throwable)null);
   }
