...
public class SvnXpp3Reader {
   ...
   private Settings read(XmlPullParser parser, boolean strict) throws IOException, XmlPullParserException {
      for(int eventType = parser.getEventType(); eventType != 1; eventType = parser.next()) {
         if (eventType == 2) {
            if (strict && !"svn-settings".equals(parser.getName())) {
               throw new XmlPullParserException("Expected root element 'svn-settings' but found '" + parser.getName() + "'", parser, (Throwable)null);
            }
      ...
      throw new XmlPullParserException("Expected root element 'svn-settings' but found no element at all: invalid XML document", parser, (Throwable)null);
   }
