...
public class AccuRevUpdateCommand extends AbstractAccuRevCommand {
   ...
   protected ScmResult executeAccurevCommand(AccuRevScmProviderRepository repository, ScmFileSet fileSet, CommandParameters parameters) throws ScmException, AccuRevException {
      ...
      if (!info.isWorkSpace()) {
         ...
         return updatedFiles != null ? new AccuRevUpdateScmResult(accuRev.getCommandLines(), getScmFiles(updatedFiles, ScmFileStatus.UPDATED), startRevision, endRevision) : new AccuRevUpdateScmResult(accuRev.getCommandLines(), "AccuRev error", accuRev.getErrorOutput(), (String)null, (String)null, false);
      }
   }
