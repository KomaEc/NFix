...
public class ProfilesXpp3Reader {
   ...
   private ProfilesRoot parseProfilesRoot(String tagName, XmlPullParser parser, boolean strict) throws IOException, XmlPullParserException {
      ...
      for(boolean foundRoot = false; eventType != 1; eventType = parser.next()) {
         if (eventType == 2) {
            if (parser.getName().equals(tagName)) {
               ...
               if (this.checkFieldWithDuplicate(parser, "profiles", (String)null, parsed)) {
                  activeProfiles = new ArrayList();
                  profilesRoot.setProfiles(activeProfiles);
                  ...
                  while(true) {
                     while(true) {
                        ...
                        if (parser.getName().equals("profile")) {
                           activeProfiles.add(this.parseProfile("profile", parser, strict));
                        } else {
                           if (strict) {
                              throw new XmlPullParserException("Unrecognised association: '" + parser.getName() + "'", parser, (Throwable)null);
                           }
