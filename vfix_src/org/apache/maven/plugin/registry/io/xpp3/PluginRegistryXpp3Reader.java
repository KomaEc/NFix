...
public class PluginRegistryXpp3Reader {
   ...
   private Plugin parsePlugin(String tagName, XmlPullParser parser, boolean strict) throws IOException, XmlPullParserException {
      ...
      while(true) {
         ...
         while(parser.nextTag() == 2) {
            if (this.checkFieldWithDuplicate(parser, "groupId", (String)null, parsed)) {
            ...
            } else if (this.checkFieldWithDuplicate(parser, "lastChecked", (String)null, parsed)) {
               plugin.setLastChecked(this.getTrimmedValue(parser.nextText()));
            } else if (this.checkFieldWithDuplicate(parser, "useVersion", (String)null, parsed)) {
               plugin.setUseVersion(this.getTrimmedValue(parser.nextText()));
            } else if (!this.checkFieldWithDuplicate(parser, "rejectedVersions", (String)null, parsed)) {
               if (strict) {
               ...
               List rejectedVersions = new ArrayList();
   ...
   private PluginRegistry parsePluginRegistry(String tagName, XmlPullParser parser, boolean strict) throws IOException, XmlPullParserException {
      ...
      for(boolean foundRoot = false; eventType != 1; eventType = parser.next()) {
         if (eventType == 2) {
            if (parser.getName().equals(tagName)) {
               ...
               if (this.checkFieldWithDuplicate(parser, "updateInterval", (String)null, parsed)) {
                  pluginRegistry.setUpdateInterval(this.getTrimmedValue(parser.nextText()));
               } else if (this.checkFieldWithDuplicate(parser, "autoUpdate", (String)null, parsed)) {
                  pluginRegistry.setAutoUpdate(this.getTrimmedValue(parser.nextText()));
               } else if (this.checkFieldWithDuplicate(parser, "checkLatest", (String)null, parsed)) {
   ...
   public PluginRegistry read(Reader reader, boolean strict) throws IOException, XmlPullParserException {
   ...
   public PluginRegistry read(Reader reader) throws IOException, XmlPullParserException {
      return this.read(reader, true);
   }

   public PluginRegistry read(InputStream in, boolean strict) throws IOException, XmlPullParserException {
      Reader reader = ReaderFactory.newXmlReader(in);
      return this.read((Reader)reader, strict);
   }

   public PluginRegistry read(InputStream in) throws IOException, XmlPullParserException {
      Reader reader = ReaderFactory.newXmlReader(in);
