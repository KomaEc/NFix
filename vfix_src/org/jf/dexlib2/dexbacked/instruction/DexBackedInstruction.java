...
public abstract class DexBackedInstruction implements Instruction {
   ...
   public static Instruction readFrom(@Nonnull DexReader reader) {
      ...
      Instruction instruction = buildInstruction((DexBackedDexFile)reader.dexBuf, opcode, reader.getOffset());
   ...
   private static DexBackedInstruction buildInstruction(@Nonnull DexBackedDexFile dexFile, @Nullable Opcode opcode, int instructionStartOffset) {
      if (opcode == null) {
         ...
         default:
            throw new ExceptionWithContext("Unexpected opcode format: %s", new Object[]{opcode.format.toString()});
         }
      }
   }
}
