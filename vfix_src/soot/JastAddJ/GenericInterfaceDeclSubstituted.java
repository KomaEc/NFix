...
public class GenericInterfaceDeclSubstituted extends GenericInterfaceDecl implements Cloneable, MemberSubstitutor {
   ...
   public GenericInterfaceDeclSubstituted copy() {
      try {
         GenericInterfaceDeclSubstituted node = this.clone();
         node.parent = null;
   ...
   public GenericInterfaceDeclSubstituted fullCopy() {
      GenericInterfaceDeclSubstituted tree = this.copy();
      if (this.children != null) {
   ...
   public GenericInterfaceDeclSubstituted(Modifiers p0, Symbol p1, List<Access> p2, List<TypeVariable> p3, TypeDecl p4) {
      this.setChild(p0, 0);
      this.setID(p1);
      this.setChild(p2, 1);
      this.setChild(p3, 2);
   ...
   public List<TypeVariable> getTypeParameterList() {
      List<TypeVariable> list = (List)this.getChild(2);
      list.getNumChild();
      return list;
   }
   ...
   public boolean supertypeGenericInterfaceDeclSubstituted(GenericInterfaceDeclSubstituted type) {
      ASTNode$State state = this.state();
      return this.original() == type.original() && type.enclosingType().subtype(this.enclosingType()) || super.supertypeGenericInterfaceDeclSubstituted(type);
   }

   public boolean supertypeGenericInterfaceDecl(GenericInterfaceDecl type) {
      ASTNode$State state = this.state();
      return super.supertypeGenericInterfaceDecl(type) || this.original().supertypeGenericInterfaceDecl(type);
   }
