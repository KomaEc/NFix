...
public class LongType extends IntegralType implements Cloneable {
   ...
   public LongType(Modifiers p0, Symbol p1, Opt<Access> p2, List<BodyDecl> p3) {
      this.setChild(p0, 0);
      this.setID(p1);
      this.setChild(p2, 1);
      this.setChild(p3, 2);
   ...
   public Constant lshift(Constant c1, Constant c2) {
      ASTNode$State state = this.state();
      return Constant.create(c1.longValue() << (int)c2.longValue());
   }
   ...
   private TypeDecl boxed_compute() {
      return this.lookupType("java.lang", "Long");
   }
