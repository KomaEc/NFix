...
public class WildcardExtendsType extends AbstractWildcardType implements Cloneable {
   ...
   public WildcardExtendsType copy() {
      try {
         WildcardExtendsType node = this.clone();
         node.parent = null;
   ...
   public WildcardExtendsType fullCopy() {
      WildcardExtendsType tree = this.copy();
      if (this.children != null) {
   ...
   public Access substitute(Parameterization parTypeDecl) {
      return (Access)(!this.usesTypeVariable() ? super.substitute(parTypeDecl) : new WildcardExtends(this.getAccess().type().substitute(parTypeDecl)));
   }
   ...
   public WildcardExtendsType(Modifiers p0, Symbol p1, List<BodyDecl> p2, Access p3) {
      this.setChild(p0, 0);
      this.setID(p1);
      this.setChild(p2, 1);
      this.setChild(p3, 2);
   ...
   private boolean involvesTypeParameters_compute() {
      return this.extendsType().involvesTypeParameters();
   }

   public boolean sameSignature(Access a) {
      ASTNode$State state = this.state();
      return a instanceof WildcardExtends ? this.getAccess().type().sameSignature(((WildcardExtends)a).getAccess()) : super.sameSignature(a);
   }
   ...
   public boolean supertypeWildcard(WildcardType type) {
      ASTNode$State state = this.state();
      return this.typeObject().subtype(this);
   }
   ...
   public boolean supertypeWildcardExtends(WildcardExtendsType type) {
      ASTNode$State state = this.state();
      return type.extendsType().subtype(this.extendsType());
   }

   public boolean supertypeClassDecl(ClassDecl type) {
      ASTNode$State state = this.state();
      return type.subtype(this.extendsType());
   }
   ...
   private boolean containedIn_compute(TypeDecl type) {
      if (type != this && !(type instanceof WildcardType)) {
         return type instanceof WildcardExtendsType ? this.extendsType().subtype(((WildcardExtendsType)type).extendsType()) : false;
      } else {
         return true;
      }
   }
   ...
   private boolean sameStructure_compute(TypeDecl t) {
      return super.sameStructure(t) || t instanceof WildcardExtendsType && ((WildcardExtendsType)t).extendsType().sameStructure(this.extendsType());
   }
