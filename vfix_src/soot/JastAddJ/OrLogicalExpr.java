...
public class OrLogicalExpr extends LogicalExpr implements Cloneable {
   ...
   public void emitEvalBranch(Body b) {
      ...
      if (this.getLeftOperand().canBeFalse()) {
         this.getRightOperand().emitEvalBranch(b);
         if (this.getRightOperand().canBeFalse()) {
   ...
   private boolean isDAafterTrue_compute(Variable v) {
      return this.getLeftOperand().isDAafterTrue(v) && this.getRightOperand().isDAafterTrue(v) || this.isFalse();
   }
   ...
   private boolean isDAafterFalse_compute(Variable v) {
      return this.getRightOperand().isDAafterFalse(v) || this.isTrue();
   }
   ...
   public boolean isDUafterTrue(Variable v) {
      ASTNode$State state = this.state();
      return this.getLeftOperand().isDUafterTrue(v) && this.getRightOperand().isDUafterTrue(v);
   }

   public boolean isDUafterFalse(Variable v) {
      ASTNode$State state = this.state();
      return this.getRightOperand().isDUafterFalse(v);
   }
   ...
   public boolean Define_boolean_isDAbefore(ASTNode caller, ASTNode child, Variable v) {
      if (caller == this.getRightOperandNoTransform()) {
         return this.getLeftOperand().isDAafterFalse(v);
      } else {
   ...
   public boolean Define_boolean_isDUbefore(ASTNode caller, ASTNode child, Variable v) {
      if (caller == this.getRightOperandNoTransform()) {
         return this.getLeftOperand().isDUafterFalse(v);
      } else {
   ...
   public soot.jimple.Stmt Define_soot_jimple_Stmt_condition_false_label(ASTNode caller, ASTNode child) {
      if (caller == this.getRightOperandNoTransform()) {
         ...
         return caller == this.getLeftOperandNoTransform() ? this.next_test_label() : this.getParent().Define_soot_jimple_Stmt_condition_false_label(this, caller);
      }
   }
