...
public class GInterfaceInvokeExpr extends AbstractInterfaceInvokeExpr implements Precedence {
   ...
   public Object clone() {
      List argList = new ArrayList(this.getArgCount());

      for(int i = 0; i < this.getArgCount(); ++i) {
         argList.add(i, Grimp.cloneIfNecessary(this.getArg(i)));
      }

      return new GInterfaceInvokeExpr(Grimp.cloneIfNecessary(this.getBase()), this.methodRef, argList);
   }
}
