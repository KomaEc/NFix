         ...
         while(true) {
            ...
            do {
               ...
               do {
                  ...
                  u = (Unit)var3.next();
                  errorSuffix = " at " + u + " in " + body.getMethod();
                  if (u instanceof DefinitionStmt) {
                     ...
                     if (!(astmt.getRightOp() instanceof CaughtExceptionRef)) {
                        ...
                        this.checkCopy(astmt, body, exception, leftType, rightType, errorSuffix);
                     }
                  }
            ...
            if (iexpr instanceof InstanceInvokeExpr) {
               ...
               this.checkCopy(stmt, body, exception, called.declaringClass().getType(), iiexpr.getBase().getType(), " in receiver of call" + errorSuffix);
            }
            ...
            if (called.parameterTypes().size() != iexpr.getArgCount()) {
               ...
               for(int i = 0; i < iexpr.getArgCount(); ++i) {
                  this.checkCopy(stmt, body, exception, Type.toMachineType(called.parameterType(i)), Type.toMachineType(iexpr.getArg(i).getType()), " in argument " + i + " of call" + errorSuffix + " (Note: Parameters are zero-indexed)");
               }
            }
         }
      }
            ...
            if (leftType instanceof RefType && rightType instanceof RefType) {
               ...
               if (!leftClass.isPhantom() && !rightClass.isPhantom()) {
                  if (leftClass.isInterface()) {
                     ...
                     exception.add(new ValidationException(stmt, "Warning: trying to use interface type where non-Object class expected" + errorSuffix + " in " + body.getMethod()));
