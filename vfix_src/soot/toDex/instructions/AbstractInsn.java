...
public abstract class AbstractInsn implements Insn {
   ...
   public int getMinimumRegsNeeded() {
      ...
      if (hasResult && incompatRegs.get(0)) {
         resultNeed = SootToDexUtils.getDexWords(((Register)this.regs.get(0)).getType());
      }

      for(int i = hasResult ? 1 : 0; i < this.regs.size(); ++i) {
         if (incompatRegs.get(i)) {
            miscRegsNeed += SootToDexUtils.getDexWords(((Register)this.regs.get(i)).getType());
         }
      }
   ...
   public String toString() {
      return this.opc.toString() + " " + this.regs;
   }
