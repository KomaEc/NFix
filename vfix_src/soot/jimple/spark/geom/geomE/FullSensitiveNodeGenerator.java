...
public class FullSensitiveNodeGenerator extends IEncodingBroker {
   ...
   public void initFlowGraph(GeomPointsTo ptAnalyzer) {
      ...
      while(true) {
         ...
         case 0:
            if (code == 0) {
               my_rhs.add_points_to_3((AllocNode)my_lhs.getWrappedNode(), 1L, 1L, ptAnalyzer.context_size[nf1]);
            } else {
               my_rhs.add_points_to_4((AllocNode)my_lhs.getWrappedNode(), 1L, 1L, ptAnalyzer.context_size[nf2], ptAnalyzer.context_size[nf1]);
            }
         ...
         case 1:
            if (cons.interCallEdges != null) {
               ...
               while(true) {
                  while(true) {
                     while(true) {
                        ...
                        if (nf2 == q.t) {
                           if (nf1 == 0) {
                              my_lhs.add_simple_constraint_4(my_rhs, 1L, q.map_offset, 1L, ptAnalyzer.max_context_size_block[q.s]);
                           } else if (q.s == q.t) {
               ...
               if (code == 0) {
                  ...
                  my_lhs.add_simple_constraint_4(my_rhs, 1L, 1L, ptAnalyzer.context_size[nf1], ptAnalyzer.context_size[nf2]);
               }
               break;
            }
         case 2:
