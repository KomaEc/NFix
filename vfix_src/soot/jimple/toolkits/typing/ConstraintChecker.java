      ...
      if (ie instanceof InterfaceInvokeExpr) {
         ...
         for(int i = 0; i < die.getBootstrapArgCount(); ++i) {
            if (die.getBootstrapArg(i) instanceof Local) {
               ...
               if (!this.hierarchy.typeNode(local.getType()).hasAncestorOrSelf(this.hierarchy.typeNode(bootstrapMethod.parameterType(i)))) {
                  if (this.fix) {
                     ie.setArg(i, this.insertCast(local, bootstrapMethod.parameterType(i), invokestmt));
                  } else {
                     error("Type Error");
                  }
               }
