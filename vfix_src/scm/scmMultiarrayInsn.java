...
class scmMultiarrayInsn extends Procedure implements Obj {
   Obj apply(Cell args, Env f) throws Exception {
      if (args == null) {
         throw new SchemeError("multianewarray expects 2 arguments");
      } else {
         Obj tmp = args.car != null ? args.car.eval(f) : null;
         Cell t = args.cdr;
         if (tmp != null && !(tmp instanceof primnode)) {
            throw new SchemeError("multianewarray expects a CP for arg #1");
            ...
            if (t == null) {
               ...
               if (!(tmp instanceof Selfrep)) {
                  throw new SchemeError("multianewarray expects a number for arg #2");
               } else {
